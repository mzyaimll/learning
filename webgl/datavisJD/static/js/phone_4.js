this.jd = this.jd||{};
(function(){
    var Phone_4 = function()
    {
        this.init();
    };

    var p = Phone_4.prototype;
    p.topNum = 10;
    p.showNum = 10;
    p.space = 96.5;
    ...
    p.initLoader = function()
    {
        var cur = this;
        var resourcesAry = [
            {id:"title", type:"texture", url:"static/images/phone_4/title.png"},
            {id:"bgLine", type:"texture", url:"static/images/phone_4/bgLine.png"},
            {id:"bottom", type:"texture", url:"static/images/phone_4/bottom.png"},
    ...
    {
        var cur = this;
        //var apiUrl =  ["data/phone_4.json", "data/phone_4_2.json"];
        var apiUrl = "http://data.jd.com/digitalBUData?type=api12_c975c5d8_5e0a_4955_a89a_8fcb9909e20f";
    ...
    p.createColumnGeometry = function()
    {
        this.columnH = 320;
        this.columnR = 25;
        this.columnGeometry = new THREE.CylinderGeometry( this.columnR, this.columnR, this.columnH, 18, 55 );
        //

        var angleSegments = 13;
        var R =this.columnR;
        var rSegments =3;
    ...
    p.createColumn = function()
    {
        ...
        this.columnContainer.position.set(-420,-200,0);
        for(var i=0; i<this.topNum; i++)
        {
            ...
            column.position.x = this.space*i;
            column.rotation.y = 6;
            var s = 1-i*0.04;
    ...
    p.columnItemAppear = function(column,i)
    {
        ...
        TweenLite.to(column.scale, 1, { delay:delayTime, x:1,z:1});
    ...
    p.createInnerParticle = function()
    {
        ...
        for(var i=0;i<this.topNum; i++)
        {
            var innerParticleItem = this.createInnerParticleItem(i,(this.topNum-i)*30+300);
            this.columnContainer.add(innerParticleItem);
            this.innerParticleItemAry.push(innerParticleItem);
            innerParticleItem.position.x = this.space*i;
        }
    };

    p.createInnerParticleItem = function(id,num)
    {
        var particleNum = num;
        var positionAry = new Float32Array(particleNum*3);
        ...
        while(i3>0)
        {
            ...
            i3 -=3;
            var s = 1-id*0.05;
            var r = Math.random()*this.columnR+1;
    ...
    p.innerParticleItemRender = function(innerParticle,id)
    {
        ...
        while(i3>0)
        {
            ...
            i3 -=3;
            //最高点
            var tempH = this.columnH * (1-id*0.04);
            //
            positionAry[i3+1] += vY[i];

            if(positionAry[i3+1]>tempH-20)
            {
                opacityAry[i] -=0.08;
            }
    ...
    p.createBgLine = function()
    {
        ...
        this.bgLine.position.x = -500;
        this.bgLine.position.y = -20;
    };
    ...
    /*------------------------------------------------- bottom start-------------------------------------------------*/
    p.createBottom = function()
    {
        for(var i=0;i<this.topNum;i++)
        {
            var bottomMC = this.createbottomItem();
            this.columnContainer.add(bottomMC);
            bottomMC.position.x = i*this.space;
            bottomMC.rotation.x = 60*(Math.PI/180);
        }
    };

    p.createbottomItem = function()
    {
        var texture = this.resourcesMap["bottom"]["result"];
        var w = texture.image.width;
        var h = texture.image.height;
        var geometry = new THREE.PlaneGeometry( w, h, 5,5 );
        var material = new THREE.MeshBasicMaterial( { map: texture,opacity:1/*,alphaMap:map*/, side: THREE.DoubleSide,transparent:true } );
        var bottomMC = new THREE.Mesh( geometry, material );
        return bottomMC
    };
    ...
    p.createNumLine = function()
    {
        ...
        for(var i=0;i<this.showNum;i++)
        {
            ...
            numLineItem.userData["endY"] =330*(1-i*0.04);
        }
    };
    ...
    {
        var delayTime = 1.5+(this.topNum-i)*0.1;
    ...
    p.createNum = function()
    {
        ...
        for(var i=0;i<this.showNum;i++)
        {
            ...
            numItem.position.y =380*(1-i*0.04);
            console.log(numItem.position.y);
            //
    ...
    p.mouseX = 0;
    p.mouseY = 0;
    p.initMouseMove = function()
    {
        var cur = this;
        document.addEventListener( 'mousemove', function(event){
            cur.mouseX = ( event.clientX - window.innerWidth / 2 );
            cur.mouseY = ( event.clientY - window.innerHeight / 2 );

        }, false );
    };

    p.mouseMoveRender = function()
    {
        //this.container.rotation.y += (this.mouseX*0.0001-this.container.rotation.y)*0.08;
        //this.container.rotation.x += (this.mouseY*0.0001-this.container.rotation.x)*0.08;
    };
    /*-------------------------------------------------- mouse end--------------------------------------------------*/
    //=================================================   threejs    end ==============================================//
    jd.Phone_4 = Phone_4;
